...
RegisterNetEvent("qb-vape:client:use", function()
    ...
    if (DoesEntityExist(ped) and not IsEntityDead(ped)) and canVape then
        canVape = false -- Start cooldown
        vaping = true
        ...
        Citizen.Wait(3000) -- Cooldown of 10 seconds
        canVape = true -- Reset cooldown
    else
       QBCore.Functions.Notify('The vape is too hot to use right now.', 'error')
    end
end)
