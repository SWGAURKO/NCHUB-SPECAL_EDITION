...
--#region ptfx
RegisterNetEvent("wsemotes:ptfx:sync", function(asset, name, offset, rot, bone, scale, color)
    if type(asset) ~= "string" or type(name) ~= "string" or type(offset) ~= "vector3" or type(rot) ~= "vector3" then
        print("[wsemotes] ptfx:sync: invalid arguments for source:", source)
        return
    end
...
RegisterNetEvent("wsemotes:ptfx:syncProp", function(propNet)
    local srcPlayerState = Player(source).state
    if propNet then
