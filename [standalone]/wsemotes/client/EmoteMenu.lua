...
function AddInfoMenu(menu)
    ...
    infomenu:AddItem(NativeUI.CreateItem("<font color=\"#FF25B1\">TayMcKenzieNZ 🇳🇿</font>",
        "<font color=\"#FF25B1\">TayMcKenzieNZ 🇳🇿</font> Project Manager for wsemotes"))
...
function OpenEmoteMenu()
    ...
    if (IsPedSwimming(playerPed) or IsPedSwimmingUnderWater(playerPed)) and not Config.AllowInWater then
        -- show in chat
        TriggerEvent('chat:addMessage', {
            color = {255, 0, 0},
            multiline = true,
    ...
    if IsPedInAnyVehicle(playerPed, false) and vehicle ~= 0 then
        -- show notification
        QBCore.Functions.Notify('You can not open the menu in the vehicle', 'error')
        return
    end
    ...
    if data.metadata['ishandcuffed'] then
        -- show notification
        QBCore.Functions.Notify('You cannot open the menu you are handcuffed.', 'error')
        return
    end
