...
Citizen.CreateThread(function()
    ...
    TriggerEvent('chat:addSuggestion', '/emotemenu', 'Open wsemotes menu (F4) by default. This may differ from server to server.')
...
if Config.MenuKeybindEnabled then
    ...
    RegisterKeyMapping("emoteui", "Open wsemotes menu", "keyboard", Config.MenuKeybind)
else
    ...
    if #args > 0 then
        if IsEntityDead(PlayerPedId()) then
            TriggerEvent('chat:addMessage', {
                color = {255, 0, 0},
                multiline = true,
                args = {"wsemotes", Config.Languages[lang]['dead']}
            })
            return
        end
        if (IsPedSwimming(PlayerPedId()) or IsPedSwimmingUnderWater(PlayerPedId())) and not Config.AllowInWater then
            TriggerEvent('chat:addMessage', {
                color = {255, 0, 0},
                multiline = true,
                args = {"wsemotes", Config.Languages[lang]['swimming']}
            })
            return
        end
...
function OnEmotePlay(EmoteName, name, textureVariation)
    ...
    if animOption then
        ...
        if animOption.PtfxAsset then
            ...
            TriggerServerEvent("wsemotes:ptfx:sync", PtfxAsset, PtfxName, vector3(Ptfx1, Ptfx2, Ptfx3), vector3(Ptfx4, Ptfx5, Ptfx6), PtfxBone, PtfxScale, PtfxColor)
        else
    ...
    if animOption and animOption.Prop then
        ...
        if animOption.PtfxAsset and not PtfxNoProp then
            TriggerServerEvent("wsemotes:ptfx:syncProp", ObjToNet(prop))
        end
    end
end
