...
QBCore.Functions.CreateCallback("Renewed-Banking:server:transfer", function(source, cb, data)
    local Player = QBCore.Functions.GetPlayer(source)
    local amount = tonumber(data.amount)
    print(QBCore.Functions.GetPlayer(tonumber(data.stateid)))
    if not QBCore.Functions.GetPlayer(tonumber(data.stateid)) then
        cb(false)
        return
    end
...
QBCore.Commands.Add('givecrypto', 'Give crypto to player.', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Amount'}}, true, function(source, args)
    local src = source
	...
	if id and amount then
				...
				if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(id))) < distance then
					if xPlayer.Functions.RemoveMoney('crypto', amount) then
						if xReciv.Functions.AddMoney('crypto', amount) then
                            ...
                            TriggerEvent('qb-logs:server:createLog', 'playermoney', '**__Give Crypto__**', GetPlayerName(src).. " Give Crypto To "..GetPlayerName(tonumber(args[1])).." "..tostring(args[2]).." " , false, src)
						else
		...
		TriggerClientEvent('QBCore:Notify', src, "Usage /givecrypto [ID] [AMOUNT]", "error")
	end
end)
