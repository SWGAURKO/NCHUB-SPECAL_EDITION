...
function LockpickIgnition(isAdvanced)
    if not HasKey then 
        ...
        QBCore.Functions.Progressbar("lockpick_ignition", "Lockicking..", lockpickTime, false, true, {
            ...
            exports['skillchecks']:lockpick(function (result)
                if result then
            ...
            end, "Name", "Placeholder!", 40000, 12, 5)
        end, function() -- Cancel
            StopAnimTask(PlayerPedId(), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
            HasKey = false
            SetVehicleEngineOn(veh, false, false, true)
...
function TriggerCallback(name, cb, ...)
    ...
    TriggerServerEvent('nc-garage:server:triggerCallback', name, ...)
end

RegisterNetEvent('nc-garage:client:triggerCallback', function(name, ...)
