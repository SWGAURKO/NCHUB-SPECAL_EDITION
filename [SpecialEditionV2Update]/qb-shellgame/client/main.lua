            ...
            if dist < Config.SpawnDistance then
                ...
                if v.spawned then
                    if currentgame == k then
                        choosetime = false
                        TriggerServerEvent('nchub-shellgame:server:setstate', currentgame, false)
                    end
                ...
                if dist < 3 and not v.arrested then
                    ...
                    if v.spawned and not v.bussy then
                        ...
                        if IsControlJustPressed(0, 38) and dist < 1.5 then
                            QBCore.Functions.TriggerCallback('nchub-shellgame:server:HasMoney', function(HasMoney)
                                if not HasMoney then
                                    ...
                                    Wait(1000)
                                    TriggerServerEvent('nchub-shellgame:server:play', v.betamount)
                                    playspeech(k, "MINIGAME_DEALER_GREET")
                                    playanim(ped, 'random@domestic', 'pickup_low', 1100)
                                    currentgame = k
                                    choosetime = false
                                    TriggerServerEvent('nchub-shellgame:server:setstate', k, true)
                        ...
                        if IsControlJustReleased(0, 47) and dist < 1.5 and PlayerJob.name == 'police' and onDuty and not v.bussy then
                            ...
                            exports['qb-target']:RemoveZone("b2"..k)
                            exports['qb-target']:RemoveZone("b3"..k)
                            TriggerServerEvent('nchub-shellgame:server:arrested', k, true)
                            SetTimeout(1000*60*Config.ArrestTimeOut, function()
                                TriggerServerEvent('nchub-shellgame:server:arrested', k, false)
                            end)
                        end
                    end
                -- else
...
exports.interact:AddInteraction({
    coords = vector3(192.7703, -944.357, 31.11287),
    distance = 3.0, -- optional
    interactDst = 1.5, -- optional
    id = 'shellgame', -- needed for removing interactions
    name = 'shellgame', -- optional
    options = {
        {
            label = 'Play for - [$85]',
            event = "",
...
function playGame(k)
    ...
    Targets["b1"..k] = exports['qb-target']:AddBoxZone("b1"..k, b1, 0.2, 0.2, {
        ...
        options = {
            {
                type = "client",
                event = "nchub-shellgame:client:choose",
                icon = "fa fa-sort-numeric-desc",
                label = Lang:t('info.choose', {value = tostring(1)}),
                action = function()
                    TriggerEvent("nchub-shellgame:client:choose", k, 1)
                end,
        ...
        options = {
            {
                ...
                action = function()
                    TriggerEvent("nchub-shellgame:client:choose", k, 2)
                end,
                canInteract = function()
                    return choosetime
                end,
        ...
        options = {
            {
                ...
                action = function()
                    TriggerEvent("nchub-shellgame:client:choose", k, 3)
                end,
                canInteract = function()
                    return choosetime
                end,
...
RegisterNetEvent('nchub-shellgame:client:setstate', function(id, state)
    Config.Locations[id].bussy = state
end)

RegisterNetEvent('nchub-shellgame:client:arrested', function(id, state)
...
RegisterNetEvent('nchub-shellgame:client:choose', function(k, id)
    ...
    if r == id then
        ...
        TriggerServerEvent('nchub-shellgame:server:win', Config.Locations[k].betamount)
    ...
    DeleteObject(bowls['b2'..k])
    DeleteObject(bowls['b3'..k])
    TriggerServerEvent('nchub-shellgame:server:setstate', k, false)
    choosetime = false
    readytoplay = false
    TaskStartScenarioInPlace(peds[k], 'WORLD_HUMAN_AA_SMOKE', 0, true)
end)
