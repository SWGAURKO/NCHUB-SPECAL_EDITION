...
Please report any issues you have in the GitHub [Issues](https://github.com/AvarianKnight/qb-voice/issues)
...
Please do not override `NetworkSetTalkerProximity`, `MumbleSetAudioInputDistance`, `MumbleSetAudioOutputDistance` or `NetworkSetVoiceActive` in any of your other scripts as there have been cases where it breaks qb-voice.
...
### PLEASE NOTE: Any keybind changes only affect new players, if you want to change your key bind go to Key Bindings -> FiveM -> Look for keybinds under 'qb-voice'.
...
qb-voice comes with a built in /muteply (tgtPly) (duration) command, in order to allow your staff to use it you will have to grand them the ace!
...
|--------------------------|--------------------------------------------------------------|----------------|
| [qb-voice:settingsCallback](docs/client-getters/events.md) | When emited it will return the current qb-voice settings. | cb(voiceSettings) |
| [qb-voice:radioActive](docs/client-getters/events.md) | Triggered when the radio is activated / deactivated | boolean |
| [qb-voice:setTalkingMode](docs/client-getters/events.md) | Triggered on proximity mode change with the voice mode id | int |
