...
function addPlayerToRadio(source, radioChannel)
	if not canJoinChannel(source, radioChannel) then
		...
		return TriggerClientEvent('qb-voice:removePlayerFromRadio', source, source)
	end
	...
	for player, _ in pairs(radioData[radioChannel]) do
		TriggerClientEvent('qb-voice:addPlayerToRadio', player, source, plyName)
	end
	voiceData[source] = voiceData[source] or defaultTable(source)
	voiceData[source].radio = radioChannel
	radioData[radioChannel][source] = false
	TriggerClientEvent('qb-voice:syncRadioData', source, radioData[radioChannel], GetConvarInt("voice_syncPlayerNames", 0) == 1 and plyName)
end
