...
--         local hasradar = v.Functions.GetItemByName('signalradar') ~= nil and true or false
--         if v and (v.PlayerData.job.name == "police" or v.PlayerData.job.name == "ambulance") and v.PlayerData.job.onduty and hasradar == true then
...
local function UpdateBlips()
    ...
    for _, v in pairs(players) do
        if v and (v.PlayerData.job.name == "police" or v.PlayerData.job.name == "ambulance") and v.PlayerData.job.onduty then
            local hasRadar = v.Functions.GetItemByName('signalradar')
            if hasRadar then
                local coords = GetEntityCoords(GetPlayerPed(v.PlayerData.source))
                local heading = GetEntityHeading(GetPlayerPed(v.PlayerData.source))
                dutyPlayers[#dutyPlayers+1] = {
...
QBCore.Functions.CreateUseableItem("policespikes", function(source, item)
    ...
    -- בדיקה האם השחקן משויך לג'וב של המשטרה
    if Player.PlayerData.job.name == "police" then
        TriggerClientEvent("loaf_spikestrips:spikestripMenu", src, item.name)
    else
        -- אם השחקן לא משויך לג'וב של המשטרה, ניתן להוסיף כאן את הפעולות הנדרשות במקרה של גישה לא מורשת
...
QBCore.Commands.Add("callsign", "Set your callsign (call number)", {{name="name", help="Name of your callsign"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == 'ambulance' then
        Player.Functions.SetMetaData("callsign", table.concat(args, " "))
        TriggerEvent('ws-10system:server:update',table.concat(args, " "),source)
    end
end)
...
QBCore.Commands.Add("jail", "Jail a player ( Police Only )", {{name="id", help="Player ID"},{name="time", help="Time"}}, true, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	local src = source;
	if Player.PlayerData.job.name == "police" then
        ...
        if time > 0 then
			...
			exports['ws-log']:Log("jail", src, "sented %s to jail for %s minutes", exports["ws-log"]:FetchFormatedIds(playerId), tonumber(time));
		else
            TriggerClientEvent('QBCore:Notify', source, "Time must be greater than 0", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', source, "This command is for cops only!", "error")
    end
end)
QBCore.Commands.Add("cjail", "Jail a player ( Corrections Only )", {{name="id", help="Player ID"},{name="time", help="Time"}}, true, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	local src = source;
	if Player.PlayerData.job.name == "corrections" then
        local playerId = tonumber(args[1])
        local time = tonumber(args[2])
        if time > 0 then
            TriggerClientEvent("police:client:JailCommand", source, playerId, time)
        ...
        TriggerClientEvent('QBCore:Notify', source, "This command is for corrections only!", "error")
    end
end)
QBCore.Commands.Add("unjail", "Unjail Player (Police Only)", {{name = "id", help = "Player ID"}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        ...
        exports['ws-log']:Log("jail", src, "unjailed %s from jail", exports["ws-log"]:FetchFormatedIds(playerId));
    else
        TriggerClientEvent('QBCore:Notify', src, 'For on-duty police only', 'error')
    end
end)
QBCore.Commands.Add("cunjail", "Unjail Player (Corrections Only)", {{name = "id", help = "Player ID"}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "corrections" and Player.PlayerData.job.onduty then
        local playerId = tonumber(args[1])
        TriggerClientEvent("prison:client:UnjailPerson", playerId)
        exports['ws-log']:Log("jail", src, "unjailed %s from jail", exports["ws-log"]:FetchFormatedIds(playerId));
    else
        TriggerClientEvent('QBCore:Notify', src, 'For on-duty corrections only', 'error')
    end
end)
...
function discordLog(name, message, plate, price, vehicle)
    local webhook = "https://discord.com/api/webhooks/1115671811166834760/sG2SbvN9VsXaeLT6ArJDAkCsH72opbpL0YxK3dDvil4e3Amz2i6Mb1qNFZDT7iBdv_L6"
    local data = {
        {
            ["color"] = '3553600',
            ["title"] = "**".. name .."**",
    ...
    PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({username = data['name'], embeds = data, avatar_url = data['image']}), { ['Content-Type'] = 'application/json' })
end
...
QBCore.Commands.Add("impound", "Impound With Price (Police Only)", {{name = "price", help = "Price for how much the person has to pay (may be empty)"}}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local pricelog = tostring(args[1])
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        TriggerClientEvent("police:client:ImpoundVehicle", src, false, tonumber(args[1]))
       -- discordLog('sk-logs', "Vehicle has been impounded by: " .. GetPlayerName(source))
    else
        TriggerClientEvent('QBCore:Notify', src, 'For on-duty police only', 'error')
    end
end)
...
--RegisterServerEvent("logs:sk")
--AddEventHandler("logs:sk", function(source, plate, vehicle, price)
--    local src = source
--    discordLog('sk-logs', "Vehicle has been impounded by: ** " , name , "** Plate of Vehicle: **" , plate , "** Vehicle Model: **" , vehicle , "** " , "Price: **" .. pricelog .. "**")
--end)
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if CuffedPlayer then
        if Player.Functions.GetItemByName("handcuffs") or Player.PlayerData.job.name == "police" then
            ...
            TriggerEvent('qb-logs:server:createLog', 'handcuffs', 'handcuffs', GetPlayerName(src).." Handcuffed "..GetPlayerName(playerId), false, src)
        end
    end
end)
...
QBCore.Commands.Add('evidence', 'Open evidence storage [case-number]', {}, false, function(source, args)
    -- TriggerServerEvent('qb-logs:server:createLog', 'evidenceoopen', 'Open Evidence', "", src)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    -- LIST Vector3
    local allowedCoordinates = {
        vector3(472.96127, -1007.445, 26.289972),
        vector3(1857.2565, 3690.9755, 29.812147), -- sandy
        vector3(610.82971, 2.1163148, 87.051506) -- VPD
    }
    local allowedRadius = 10.0
    ...
    if Player and (Player.PlayerData.job.name == 'police') then
        ...
        local allowed = false
        for _, coords in ipairs(allowedCoordinates) do
        ...
        if allowed then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'You must be within the allowed radius to use this command.', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'This command is for emergency services!', 'error')
    end
end)
...
-- QBCore.Commands.Add('fine', 'Fine A Person', {{name = 'id', help = 'ID'}, {name = 'amount', help = 'amount'}}, false, function(source, args)
...
--         end
--     else
--         TriggerClientEvent('QBCore:Notify', source, 'For on-duty police only', 'error')
--     end
-- end)
...
RegisterServerEvent('qb-police:server:glichy', function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local price = 150
