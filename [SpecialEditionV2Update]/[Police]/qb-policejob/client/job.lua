...
local function SetCWeaponSeries()
    for k, v in pairs(Config.CItems.items) do
        if k < 6 then
            Config.CItems.items[k].info.serie = tostring(QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) .. QBCore.Shared.RandomInt(1) .. QBCore.Shared.RandomStr(2) .. QBCore.Shared.RandomInt(3) .. QBCore.Shared.RandomStr(4))
        end
    end
end
...
RegisterNetEvent('police:client:SendEmergencyMessage', function()
    TriggerServerEvent('police:server:policeAlert', '| 10-14 | Officer in Distress')
...
RegisterNetEvent('police:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            ...
            --TriggerServerEvent("logs:sk", GetVehicleNumberPlateText(vehicle), tostring(gtr))
            TriggerServerEvent('Prime-Parking:server:removeOutsideVehicles', plate)
            TriggerServerEvent('qb-logs:server:createLog', 'police', '__**Impound**__', GetPlayerName(PlayerId()).." **Impounded Car**: __**"..plate.."**__ ".."Price: "..price, false)
            --TriggerClientEvent('qb-logs:server:createLog', -1, 'impound', 'New Item Drop', "**".. GetPlayerName(source) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..source.."*) dropped new item; name: **".. .."**, amount: **" .. itemAmount .. "**", false, source)
        end
    end
end)
...
CreateThread(function()
    ...
    while true do
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "corrections" then
            local pos = GetEntityCoords(PlayerPedId())
                    ...
                    if onDuty then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                local index = 1
                                for _, armoryItem in pairs(Config.CItems.items) do
                                ...
                                SetCWeaponSeries()
                                TriggerServerEvent("inventory:server:OpenInventory", "shop", "corrections", authorizedItems)
                            end
                        elseif #(pos - v) < 2.5 then
                    ...
                    if onDuty then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                DrawText3D(v.x, v.y, v.z, "~g~G~w~ - Store helicopter")
                            else
                                DrawText3D(v.x, v.y, v.z, "~g~G~w~ - Take a helicopter")
                            end
...
--armory
RegisterNetEvent('police:openCArmory')
AddEventHandler('police:openCArmory', function()
    if onDuty then
        local authorizedItems = {
            label = "Corrections Armory",
            slots = 30,
...
-- --stash
-- RegisterNetEvent('police:client:pstash', function()
--     TriggerServerEvent("inventory:server:OpenInventory", "stash", "policestash_" .. QBCore.Functions.GetPlayerData().citizenid)
--     TriggerEvent("inventory:client:SetCurrentStash", "policestash_" .. QBCore.Functions.GetPlayerData().citizenid)
-- end)

--fingerprint
RegisterNetEvent('police:client:fingerprint', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("police:server:showFingerprint", playerId)
...
RegisterNetEvent('qb-policejob:interact:stash', function()
...
RegisterNetEvent('qb-pawn:interact:stash', function()
    if PlayerJob.name == 'pawn' then
        TriggerServerEvent('inventory:server:OpenInventory', 'stash', 'pawnstash_'..QBCore.Functions.GetPlayerData().citizenid)
        TriggerEvent('inventory:client:SetCurrentStash', 'pawnstash_'..QBCore.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('qb-vanilla:interact:stash', function()
    if PlayerJob.name == 'vanilla' then
        TriggerServerEvent('inventory:server:OpenInventory', 'stash', 'club77_'..QBCore.Functions.GetPlayerData().citizenid)
        TriggerEvent('inventory:client:SetCurrentStash', 'club77_'..QBCore.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('qb-taco:interact:stash', function()
    if PlayerJob.name == 'taco' then
        TriggerServerEvent('inventory:server:OpenInventory', 'stash', 'taco_'..QBCore.Functions.GetPlayerData().citizenid)
        TriggerEvent('inventory:client:SetCurrentStash', 'taco_'..QBCore.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('qb-mechanic:interact:stash', function()
    if PlayerJob.name == 'mechanic' then
        TriggerServerEvent('inventory:server:OpenInventory', 'stash', 'mechanic_'..QBCore.Functions.GetPlayerData().citizenid)
        TriggerEvent('inventory:client:SetCurrentStash', 'mechanic_'..QBCore.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('qb-ccafe:interact:stash', function()
    if PlayerJob.name == 'catcafe' then
        TriggerServerEvent('inventory:server:OpenInventory', 'stash', 'catcafe_'..QBCore.Functions.GetPlayerData().citizenid)
        TriggerEvent('inventory:client:SetCurrentStash', 'catcafe_'..QBCore.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('qb-yellowjack:interact:stash', function()
    if PlayerJob.name == 'yellowjack' then
        TriggerServerEvent('inventory:server:OpenInventory', 'stash', 'yellowjack_'..QBCore.Functions.GetPlayerData().citizenid)
        TriggerEvent('inventory:client:SetCurrentStash', 'yellowjack_'..QBCore.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('qb-coolbeans:interact:stash', function()
    if PlayerJob.name == 'coolbeans' then
        TriggerServerEvent('inventory:server:OpenInventory', 'stash', 'coolbeans_'..QBCore.Functions.GetPlayerData().citizenid)
        TriggerEvent('inventory:client:SetCurrentStash', 'coolbeans_'..QBCore.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('qb-atom:interact:stash', function()
    if PlayerJob.name == 'burgershot' then
        TriggerServerEvent('inventory:server:OpenInventory', 'stash', 'atom_'..QBCore.Functions.GetPlayerData().citizenid)
        TriggerEvent('inventory:client:SetCurrentStash', 'atom_'..QBCore.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('qb-redline:interact:stash', function()
    if PlayerJob.name == 'redline' then
        TriggerServerEvent('inventory:server:OpenInventory', 'stash', 'redline_'..QBCore.Functions.GetPlayerData().citizenid)
        TriggerEvent('inventory:client:SetCurrentStash', 'redline_'..QBCore.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('qb-judge:interact:stash', function()
    if PlayerJob.name == 'judge' then
        TriggerServerEvent('inventory:server:OpenInventory', 'stash', 'judgestash_'..QBCore.Functions.GetPlayerData().citizenid)
        TriggerEvent('inventory:client:SetCurrentStash', 'judgestash_'..QBCore.Functions.GetPlayerData().citizenid)
    end
end)

RegisterNetEvent('qb-policejob:interact:vest', function()
    if PlayerJob.name == 'police' then
        local playerPed = PlayerPedId()
        RequestAnimDict('clothingtie') 
        while not HasAnimDictLoaded('clothingtie') do Wait(0) end
        TaskPlayAnim(playerPed, 'clothingtie', 'try_tie_positive_a', 3.0, 3.0, 5000, 51, 0, false, false, false)
        QBCore.Functions.Progressbar('putting_on_vest', 'Putting on vest', 5000, true, false, {
