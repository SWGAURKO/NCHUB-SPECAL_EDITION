...
--- Event [Nui]
RegisterNetEvent('nc-admin:server:getMoney', function(moneytype)
    local src = source
    local result =  MySQL.query.await('SELECT money FROM players', {})
        if result then
            ...
            TriggerClientEvent('nc-admin:client:getMoney', src, totalMoney, moneytype)
        end
end)

RegisterNetEvent('nc-admin:server:getPlayerProfile', function(player)
    ...
    TriggerClientEvent('nc-admin:client:getPlayerProfile',source,data)
end)
