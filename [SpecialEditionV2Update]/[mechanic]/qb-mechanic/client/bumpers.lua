...
RegisterNetEvent('jim-mechanic:client:Bumpers:Apply', function(data)
	...
	elseif GetVehicleMod(vehicle, tonumber(data.bumperid)) ~= tonumber(data.mod) then
		...
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			TriggerServerEvent('qb-logs:server:createLog', 'bumpers', 'Bumpers', "", src)
...
-- Till Stuff --
RegisterNetEvent("qb-mechanicc:bill")
AddEventHandler("qb-mechanicc:bill", function()
    ...
    if bill ~= nil then
        ...
        TriggerServerEvent("qb-mechanicc:bill:player", bill[1].input, bill[2].input)
    end
end)
