...
RegisterNetEvent('nc-notebook:showNotebook:client', function(metadata, slot)
    local mainData = {}
...
RegisterNUICallback('callback', function(data)
    if data.action == "nuiFocus" then
        ...
        TriggerServerEvent('nc-notebook:updateMetadata:server', json.encode(data.metadata), currentItemSlot, data.nheader, data.ndesc)
    end
end)