...
local function RepairVehicleFull(vehicle)
	...
	TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items["repairkit"], "remove")
	local pickSuccess = exports['qb-lock']:StartLockPickCircle(math.random(2, 4), math.random(55, 75))

	if pickSuccess then
		QBCore.Functions.Progressbar("repair_vehicle", "Repairing vehicle..", math.random(10000, 15000), false, true, {
			disableMovement = true,
			disableCarMovement = true,
			disableMouse = false,
			disableCombat = true,
