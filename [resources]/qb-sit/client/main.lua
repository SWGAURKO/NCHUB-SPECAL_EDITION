...
CreateThread(function()
    ...
    --     exports['qb-target']:AddBoxZone('PolyChair'..k, v.coords, v.length, v.width, {
    --         name = 'PolyChair'..k,
    --         heading = v.heading,
    --         debugPoly = false,
    --         minZ = v.minZ,
...
RegisterNetEvent("qb:target:init:" .. GetCurrentResourceName(), function(Mapper)
    ...
    if not IsPedInAnyVehicle(PlayerPedId()) then
        if sitting then
            ...
            if distance < 1.5 then
                ...
                QBCore.Functions.Notify('Not Near Something To Sit On', 'error')
            end
        end
    else
        QBCore.Functions.Notify('Can\'t Do That In A Vehicle', 'error')
    end
end)

RegisterNetEvent('qb-sit:sitAlt', function(dat)
    if not IsPedInAnyVehicle(PlayerPedId()) then
        if sitting then
            Standup()
        else
...
function Standup()
    ...
    TriggerServerEvent('qb-sit:server:LeaveChair', id)
    id = 0
	currentScenario = nil
end
...
function sit(object)
    ...
    TriggerServerEvent('qb-sit:server:GetChair', id)
end

function sitAlt(dat)
    if dat.PolyCoords == nil then return end
    id = math.random(11111, 99999)
    TriggerServerEvent('qb-sit:server:GetChairAlt', id, dat)
end

RegisterNetEvent('qb-sit:client:GetChair', function(occupied)
    if occupied then
        QBCore.Functions.Notify('Chair Is Occupied', 'error')
        id = 0
        currentScenario = nil
    else
        local playerPed = PlayerPedId()
        lastPos = GetEntityCoords(playerPed)
        TriggerServerEvent('qb-sit:server:TakeChair', id)
...
RegisterNetEvent('qb-sit:client:GetChairAlt', function(occupied, dat)
    if occupied then
        QBCore.Functions.Notify('Chair Is Occupied', 'error')
        id = 0
        currentScenario = nil
...
CreateThread(function()
    TriggerEvent("qb:target:init", GetCurrentResourceName(), "Mapping")
end)

RegisterNetEvent("qb:target:ready", function()
    TriggerEvent("qb:target:init", GetCurrentResourceName(), "Mapping")
end)