...
local bennyLocation = vector3(-33.15689, -1054.989, 27.791086)
local bennyPDLocation = vector3(0, 0, 0)
local bennyspaletopd = vector3(0, 0, 0)


local currentBennys = nil

local bennyHeading = 90.2
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("qb-bennys:attemptPurchase", type, upgradeLevel, currentBennys)
...
RegisterNetEvent('event:control:bennysregular')
AddEventHandler('event:control:bennysregular', function(useID)
    useID = 1
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        if useID == 1   then -- Bennys
            enterLocation(bennyLocation)
        end
    end
end)

RegisterNetEvent('event:control:bennysregular2')
AddEventHandler('event:control:bennysregular2', function(useID)
    useID = 1
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        if useID == 1   then -- Bennys
...
RegisterNetEvent('event:control:bennysregular3')
AddEventHandler('event:control:bennysregular3', function(useID)
    useID = 1
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        if useID == 1   then -- Bennys
...
RegisterNetEvent('event:control:bennysregular4')
AddEventHandler('event:control:bennysregular4', function(useID)
    useID = 1
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        if useID == 1   then -- Bennys
...
RegisterNetEvent('event:control:bennysregulars')
AddEventHandler('event:control:bennysregulars', function(useID)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        bennyHeading = 221.79
        if useID == 1 and not isPlyInBennys then -- Paleto Bennys
            enterLocation(bennyPaletoLocation)
        end
    end
end)

RegisterNetEvent('event:control:pdbennysstam')
AddEventHandler('event:control:pdbennysstam', function(useID)
    if job == "police" and IsPedInAnyPoliceVehicle(PlayerPedId()) and IsPedInAnyVehicle(PlayerPedId(), false) then
        useID = 1
        bennyHeading = 90.13
        if useID == 1 then -- PD Bennys
            enterLocation(bennyPDLocation)
        end
    end
end)

RegisterNetEvent('event:control:paletopdbennys')
AddEventHandler('event:control:paletopdbennys', function(useID)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        bennyHeading = 313.0
        if useID == 1 and not isPlyInBennys then -- Paleto PD Bennys
            enterLocation(bennyspaletopd)
        end
    end
end)


RegisterNetEvent('event:control:adminmenu')
AddEventHandler('event:control:adminmenu', function()
    if IsPedInAnyVehicle(PlayerPedId(), false) then
            EnterBennys()
        end
end)
...
Citizen.CreateThread(function()
    while true do 
        ...
        if job == "police" and IsPedInAnyPoliceVehicle(PlayerPedId()) and IsPedInAnyVehicle(plyPed, false) then
            ...
            nearDefault = isNear(plyPos, bennyspaletopd, 10) 
                ...
                if nearDefault then
                    if not isPlyInBennys then
                        if IsControlJustReleased(1, 38) then
                            TriggerEvent('event:control:paletopdbennys', 1)
                        end
                    end
                end
            else
...
RegisterNetEvent("qb-bennys:purchaseFailedpolice")
AddEventHandler("qb-bennys:purchaseFailedpolice", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    QBCore.Functions.Notify("Not enough Money In The Bossmenu.", "error")
end)