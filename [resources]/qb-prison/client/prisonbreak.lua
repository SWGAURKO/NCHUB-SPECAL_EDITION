...
--[[Citizen.CreateThread(function()
	while true do
		...
		if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["middle"].coords.x, Config.Locations["middle"].coords.y, Config.Locations["middle"].coords.z, false) > 200.0 and inJail) then
            ...
            QBCore.Functions.Notify("You escaped, you better run quick from here...", "error")
		end
	end
end)]]--
...
AddEventHandler('prison:client:SetLockDown', function(isLockdown)
    ...
    if securityLockDown and inJail then
        QBCore.Functions.Notify("Highest security level set to active, stick to the cells blocks", 10000)
    end
end)
...
Citizen.CreateThread(function()
	while true do
    	...
    	if(Vdist(1778.47, 2557.58, 45.68, pos.x, pos.y, pos.z) < 10.0)then
			...
			if(Vdist(1778.47, 2557.58, 45.68, pos.x, pos.y, pos.z) < 2.0)then
				...
				if IsControlJustPressed(1, 38) and (Vdist(1778.47, 2557.58, 45.68, pos.x, pos.y, pos.z) < 2.0) then
					Progressbar(60000,"Making a goody goody water madafaka...")
                            TriggerServerEvent("inventory:server:OpenInventory", "shop", "jail_water" , {
                                ['label'] = "Jail",
                                ['items'] = {
                                    [1] = {
                                        name = "water",
    	...
    	if(Vdist(1782.90, 2561.38, 45.67, pos.x, pos.y, pos.z) < 10.0)then
			found = true
			if(Vdist(1782.90, 2561.38, 45.67, pos.x, pos.y, pos.z) < 2.0)then
				-- DisplayHelpText("Press ~INPUT_CONTEXT~ what dis?")
				if IsControlJustPressed(1, 38) and (Vdist(1782.90, 2561.38, 45.67, pos.x, pos.y, pos.z) < 2.0) then
        ...
        if(Vdist(1782.41, 2563.78, 45.67, pos.x, pos.y, pos.z) < 10.0)then
			found = true
			if(Vdist(1782.41, 2563.78, 45.67, pos.x, pos.y, pos.z) < 2.0)then
				-- DisplayHelpText("Press ~INPUT_CONTEXT~ what dis?")
                if IsControlJustPressed(1, 38) and (Vdist(1782.41, 2563.78, 45.67, pos.x, pos.y, pos.z) < 2.0) then
                    if HasItem('assphone') then
						Progressbar(30000,"Attempting to fix phone")
						local chance = math.random(1,100)
						if chance > 70 and HasItem('assphone') then
                            TriggerServerEvent('QBCore:Server:RemoveItem', 'assphone', 1)
                            TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["assphone"], "remove")
    	...
    	if(Vdist(1777.48, 2564.22, 45.67, pos.x, pos.y, pos.z) < 10.0)then
			found = true
			if(Vdist(1777.48, 2564.22, 45.67, pos.x, pos.y, pos.z) < 2.0)then
				-- DisplayHelpText("Press ~INPUT_CONTEXT~ what dis?")
				if IsControlJustPressed(1, 38) and (Vdist(1777.48, 2564.22, 45.67, pos.x, pos.y, pos.z) < 2.0) then
						Progressbar(60000,"Searching...")
						local chance = math.random(1,100)
						if chance > 80 then
                            TriggerServerEvent("inventory:server:OpenInventory", "shop", "jail_crack_baggy" , {
                                ['label'] = "Jail",
                                ['items'] = {
                                    [1] = {
                                        name = "crack_baggy",
    	...
    	if(Vdist(1777.53, 2560.80, 45.67, pos.x, pos.y, pos.z) < 10.0)then
			found = true
			if(Vdist(1777.53, 2560.80, 45.67, pos.x, pos.y, pos.z) < 2.0)then
				-- DisplayHelpText("Press ~INPUT_CONTEXT~ what dis?")
				if IsControlJustPressed(1, 38) and (Vdist(1777.53, 2560.80, 45.67, pos.x, pos.y, pos.z) < 2.0) then
						Progressbar(60000,"Searching...")
						local chance = math.random(1,100)
						if chance > 80 then
...
function Progressbar(duration, label)
	local retval = nil
	QBCore.Functions.Progressbar("jail", label, duration, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
