...
QBCore.Functions.CreateCallback('qb-mining:Check', function(source, cb, item, crafttable)
	local src = source
...
QBCore.Functions.CreateCallback('qb-mining:ItemCheck', function(source, cb, item, cost)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	local hasitem = false
...
---Crafting
RegisterServerEvent('qb-mining:GetItem', function(data)	
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	...
	for k,v in pairs(data.craftable[data.tablenumber][data.item]) do
		if not Player.Functions.GetItemByName(tostring(k)) then 
			DropPlayer(src, "nice try ya oompa loompa L BOZO cry about it 🤣🤣🤣🤣🤣🤣🤣\nbtw thanks for ur 5k 👀👀👀") 	
			return
		end
	...
	TriggerClientEvent("qb-mining:CraftMenu", src, data)
end)

RegisterServerEvent('qb-mining:MineReward', function()
...
RegisterServerEvent('qb-mining:CrackReward', function(cost)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
--Stone Cracking Checking Triggers
--Command here to check if any stone is in inventory
RegisterServerEvent('qb-mining:WashReward', function(cost)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.RemoveItem('stone', cost)
    TriggerClientEvent("inventory:client:ItemBox", src, QBCore.Shared.Items["stone"], "remove", 1)
...
RegisterServerEvent('qb-mining:PanReward', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	for i = 1, math.random(1,3) do
...
RegisterNetEvent("qb-mining:Selling", function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
