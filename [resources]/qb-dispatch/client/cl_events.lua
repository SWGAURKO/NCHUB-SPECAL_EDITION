...
local function VehicleShooting(vehdata)
    ...
    if not IsPoliceJob(PlayerJob.name) then
        TriggerServerEvent("dispatch:server:notify",{
...
local function CornerSell(camId)
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "corner", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-12",
        firstStreet = locationInfo,
        gender = gender,
        camId = camId,
        ...
        dispatchMessage = _U('corner'), -- message
        job = {"police"} -- jobs that will get the alerts
    })
end

exports('CornerSell', CornerSell)

local function TruckRobbery(camId)
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "truckrobbery", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-15",
        firstStreet = locationInfo,
        gender = gender,
        camId = camId,
        ...
        dispatchMessage = _U('truckrobbery'), -- message
        job = {"police"} -- jobs that will get the alerts
    })
end exports('TrainRobbery', TrainRobbery)

local function artrobberygallery(camId)
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "artrobberygallery", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-15",
        firstStreet = locationInfo,
        gender = gender,
        camId = camId,
        ...
        dispatchMessage = _U('artrobberygallery'), -- message
        job = {"police"} -- jobs that will get the alerts
    })
end exports('artrobberygallery', artrobberygallery)
RegisterNetEvent('qb-dispatch:artrobberygallery', artrobberygallery)

local function drugtrafficking(camId)
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "drugtrafficking", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-15",
        firstStreet = locationInfo,
        gender = gender,
        camId = camId,
        ...
        dispatchMessage = _U('drugtrafficking'), -- message
        job = {"police"} -- jobs that will get the alerts
    })
end exports('drugtrafficking', drugtrafficking)
RegisterNetEvent('qb-dispatch:drugtrafficking', drugtrafficking)
...
RegisterNetEvent('qb-dispatch:pacificRobbery', PacificBankRobbery)
...
local function StoreRobbery()
    ...
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "store", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-90",
        firstStreet = locationInfo,
        gender = gender,
        model = nil,
        ...
        dispatchMessage = _U('store'), -- message
        job = {"police"} -- jobs that will get the alerts
    })
end exports('StoreRobbery', StoreRobbery)
RegisterNetEvent('qb-dispatch:StoreRobbery', StoreRobbery)

local function LockPickDoor()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "lockpickdoor", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-890",
        firstStreet = locationInfo,
        gender = gender,
        model = nil,
        ...
        dispatchMessage = _U('lockpickdoor'), -- message
        job = {"police"} -- jobs that will get the alerts
    })
end exports('LockPickDoor', LockPickDoor)
RegisterNetEvent('qb-dispatch:LockPickDoor', LockPickDoor)
...
local function cornerselling()
    ...
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "cornerselling", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-90",
        firstStreet = locationInfo,
        gender = gender,
        model = nil,
        ...
        dispatchMessage = _U('cornerselling'), -- message
        job = {"police"} -- jobs that will get the alerts
    })
end exports('cornerselling', cornerselling)

RegisterNetEvent("qb-dispatch:client:truckrobbery", function()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    local heading = getCardinalDirectionFromHeading()
    ...
    TriggerServerEvent("dispatch:server:notify",{
        ...
        dispatchMessage = "Truck Robbery", -- message
        job = {"police"} -- jobs that will get the alerts
    })
end)
...
local function officerdown()
    ...
    local heading = getCardinalDirectionFromHeading()
    PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
    TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 20.0, "panic", 0.1)
    TriggerServerEvent("dispatch:server:notify",{
...
local canUseOfficerDownButton = true
local lastOfficerDownButtonTime = 0

RegisterNetEvent("qb-dispatch:client:officerdown", function()
    local plyData = QBCore.Functions.GetPlayerData()
    local hasSignalRadar = false

    -- Check if the player has the "signalradar" item
    for _, item in pairs(plyData.items) do
        if item.name == "signalradar" then
            hasSignalRadar = true
            break
        end
    end

    if hasSignalRadar then
        local currentTime = GetGameTimer()
        if canUseOfficerDownButton and (currentTime - lastOfficerDownButtonTime) >= 10000 then
            canUseOfficerDownButton = false
            lastOfficerDownButtonTime = currentTime
            Citizen.SetTimeout(10000, function()
                canUseOfficerDownButton = true
            end)
            ...
            QBCore.Functions.Notify('You need to wait 10 seconds', 'error')
        end
    else
        QBCore.Functions.Notify('You need signalradar to use the Officer Down button', 'error')
    end
end)
...
local function panicbutton()
    ...
    local heading = getCardinalDirectionFromHeading()
    PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
    TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 20.0, "panic", 0.6)
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "panicbutton", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-99",
        firstStreet = locationInfo,
        callsign = callsign,
        priority = 1,
...
end exports('panicbutton', panicbutton)


local canUsePanicButton = true
local lastPanicButtonTime = 0

RegisterNetEvent("qb-dispatch:client:panicbutton", function()
    local plyData = QBCore.Functions.GetPlayerData()
    local hasSignalRadar = false
    ...
    if hasSignalRadar then
        local currentTime = GetGameTimer()
        if canUsePanicButton and (currentTime - lastPanicButtonTime) >= 10000 then
            canUsePanicButton = false
            lastPanicButtonTime = currentTime
            Citizen.SetTimeout(10000, function()
                canUsePanicButton = true
            end)
            ...
            TriggerServerEvent("dispatch:server:notify", {
                ...
                dispatchMessage = "Panic Button", -- message
                job = {"police", "ambulance"} -- jobs that will get the alerts
            })
        else
            QBCore.Functions.Notify('You need to wait 10 seconds', 'error')
        ...
        QBCore.Functions.Notify('You need signalradar to use the panic button', 'error')
    end
end)

local canUseGPSButton = true
local lastGPSButtonTime = 0

RegisterNetEvent("qb-dispatch:client:gpsbutton", function()
    local plyData = QBCore.Functions.GetPlayerData()
    local hasSignalRadar = false
    ...
    if hasSignalRadar then
        local currentTime = GetGameTimer()
        if canUseGPSButton and (currentTime - lastGPSButtonTime) >= 10000 then
            canUseGPSButton = false
            lastGPSButtonTime = currentTime
            Citizen.SetTimeout(10000, function()
                canUseGPSButton = true
            end)
        ...
        QBCore.Functions.Notify('You need signalradar to use the GPS button', 'error')
    end
end)

local function GPSButton()
    if not cooldown then
        ...
        local callsign = QBCore.Functions.GetPlayerData().metadata["callsign"]
        TriggerServerEvent("dispatch:server:notify",{
            dispatchcodename = "gpsbutton", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
            dispatchCode = "10-99",
            firstStreet = locationInfo,
            name = plyData.charinfo.firstname:sub(1,1):upper()..plyData.charinfo.firstname:sub(2).. " ".. plyData.charinfo.lastname:sub(1,1):upper()..plyData.charinfo.lastname:sub(2),
            model = nil,
            ...
            dispatchMessage = _U('gpsbutton'), -- message
            job = {"police", "ambulance"} -- jobs that will get the alerts
        })
        cooldown = true
        Wait(1000 * 30)
...
end exports('GPSButton', GPSButton)
...
RegisterNetEvent('qb-dispatch:client:gpslocation', function()
    exports['qb-dispatch']:GPSButton()
end)