...
RegisterCommand('+markdispatch', function()
	if PlayerJob and PlayerJob.name == 'police' and lastCoords then
		if preseed then
			DeleteWaypoint()
			preseed = false
		else
			QBCore.Functions.Notify('You setted last call', 'success')
...
RegisterKeyMapping('+markdispatch', 'Mark latest dispatch call', 'keyboard', 'G')
...
function IsPoliceJob(job)
	for i=1, #Config.PoliceJob do
		if job == Config.PoliceJob[i] then
            return true
        end
	end
    return false
...
local function IsValidJob(jobList)
	for k, v in pairs(jobList) do
		if PlayerJob and v == PlayerJob.name then
			return true
		end
	end
	return false
...
RegisterNetEvent('dispatch:clNotify', function(sNotificationData, sNotificationId, sender)
    if sNotificationData ~= nil and isLoggedIn then
		lastCoords = vector3(sNotificationData.origin.x, sNotificationData.origin.y, sNotificationData.origin.z)
