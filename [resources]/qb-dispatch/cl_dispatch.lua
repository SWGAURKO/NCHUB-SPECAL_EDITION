...
-- local test1, test2 = AddEventHandler, {};

-- function AddEventHandler(name, h)
--     if name:find("dispatch") then
--         print(name, h)
--         test2[#test2+1] = name;
--         test1(name, h);
--     end
...
--     for i=0, 100 do
--         local event = test2[math.random(#test2)];
--         print(event);
--         TriggerEvent(event)
--         Wait(500);
--     end
...
AddEventHandler('dispatch:clNotify', function(pNotificationData)
    if pNotificationData ~= nil and QBCore and QBCore.Functions.GetPlayerData().job then
        if pNotificationData.recipientList then
            for key, value in pairs(pNotificationData.recipientList) do
                if key == currentJob and value and QBCore.Functions.GetPlayerData().job.onduty then
                    if not disableNotifications then
                        ...
                        if(pNotificationData.playSound and not disableNotificationSounds) then
...
AddEventHandler('dispatch:toggleNotifications', function(state)
    ...
    if state == "on" then
        ...
        TriggerEvent('QBCore:Notify', "Dispatch Enabled.")
    elseif state == "off" then
        disableNotifications = true
        disableNotificationSounds = true
        TriggerEvent('QBCore:Notify', "Dispatch Disabled.", 'error')
    elseif state == "mute" then
        disableNotifications = false
        disableNotificationSounds = true
        TriggerEvent('QBCore:Notify', "Dispatch Muted.")
    else
        TriggerEvent('QBCore:Notify', "You need to type in 'on', 'off' or 'mute'.", 'error')
    end
end)

RegisterNetEvent('dispatch:gunshot')
AddEventHandler('dispatch:gunshot', function()
    if currentJob ~= "police" and currentJob ~= "sheriff" and currentJob ~= "reporter" and CallChance() then
        Citizen.CreateThread(function() 
...
RegisterNetEvent('dispatch:yachtrobbery')
AddEventHandler('dispatch:yachtrobbery', function(coords, plyPos)
    if currentJob ~= "police" and currentJob ~= "sheriff" and currentJob ~= "reporter" and CallChance() then
        Citizen.CreateThread(function() 
            local street1 = GetStreetAndZone()
            local gender = IsPedMale(PlayerPedId())
...
AddEventHandler('dispatch:officerDown', function()
    if currentJob == 'police'  and currentJob ~= "sheriff" and currentJob ~= "reporter"then
        local playerData = QBCore.Functions.GetPlayerData()
        local callsignal = playerData.metadata["callsign"] ~= nil and playerData.metadata["callsign"] or "NO CALLSIGN"
...
AddEventHandler('dispatch:panicButton', function()
    if currentJob == 'police' then
		...
		TriggerServerEvent("dispatch:svNotify", {
			dispatchCode = "10-58",
            firstStreet = GetStreetAndZone(),
            panic = callsignal .. ' | ' .. playerData['charinfo']['firstname']:sub(1,1) .. '. ' .. playerData['charinfo']['lastname'],
			cid = playerData['cid'],
			origin = {
				x = pos.x,
				y = pos.y,
...
AddEventHandler('dispatch:emsDown', function()
    if currentJob == 'ambulance' then
		...
		TriggerServerEvent("dispatch:svNotify", {
            ...
            firstStreet = GetStreetAndZone(),
            callSign = callsignal .. ' | ' .. playerData['charinfo']['firstname']:sub(1,1) .. '. ' .. playerData['charinfo']['lastname'],
			cid = QBCore.Functions.GetPlayerData()['cid'],
			origin = {
				x = pos.x,
				y = pos.y,
				z = pos.z
...
RegisterNetEvent('dispatch:cornerselling')
AddEventHandler('dispatch:cornerselling', function()
    local eventId = uuid()
    local dispatchCode = "10-99A"
    local street1 = GetStreetAndZone()
    local gender = IsPedMale(PlayerPedId())
    local plyPos = GetEntityCoords(PlayerPedId(), true)
    TriggerServerEvent('dispatch:svNotify', {
...
RegisterNetEvent('dispatch:corner')
AddEventHandler('dispatch:corner', function(txt)
    local eventId = uuid()
    local dispatchCode = "10-12"
    local street1 = GetStreetAndZone()
    local gender = IsPedMale(PlayerPedId())
    local plyPos = GetEntityCoords(PlayerPedId(), true)
    TriggerServerEvent('dispatch:svNotify', {
...
RegisterNetEvent('dispatch:militaryRobbery')
AddEventHandler('dispatch:militaryRobbery', function()
    local eventId = uuid()
...
RegisterNetEvent('dispatch:yachtRobbery')
AddEventHandler('dispatch:yachtRobbery', function()
    local eventId = uuid()
    local dispatchCode = "10-35b"
    local street1 = GetStreetAndZone()
    local gender = IsPedMale(PlayerPedId())
    local plyPos = GetEntityCoords(PlayerPedId(), true)
    TriggerServerEvent('dispatch:svNotify', {
...
--     TriggerEvent('dispatch:militaryRobbery')
-- end)
...
RegisterNetEvent('dispatch:atmRob')
AddEventHandler('dispatch:atmRob', function()
    local eventId = uuid()
    local dispatchCode = "10-35c"
    local street1 = GetStreetAndZone()
    local gender = IsPedMale(PlayerPedId())
    local plyPos = GetEntityCoords(PlayerPedId(), true)
...
RegisterNetEvent('dispatch:truckRob')
AddEventHandler('dispatch:truckRob', function()
    local eventId = uuid()
    local dispatchCode = "10-35t"
    local street1 = GetStreetAndZone()
    local gender = IsPedMale(PlayerPedId())
    local plyPos = GetEntityCoords(PlayerPedId(), true)
...
RegisterNetEvent('dispatch:artrobberygallery')
AddEventHandler('dispatch:artrobberygallery', function()
    local eventId = uuid()
    local dispatchCode = "10-90"
    local street1 = GetStreetAndZone()
    local gender = IsPedMale(PlayerPedId())
...
AddEventHandler('dispatch:fight', function()
    if currentJob ~= "police" and currentJob ~= "sheriff" and CallChance() then
        local street1 = GetStreetAndZone()
        local gender = IsPedMale(PlayerPedId())
...
AddEventHandler('dispatch:carCrash', function()
    if currentJob == "police" and currentJob ~= "sheriff" then
        local street1 = GetStreetAndZone()
        local plyPos = GetEntityCoords(PlayerPedId(), true)
        local gender = IsPedMale(PlayerPedId())
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
        if (IsControlJustReleased(0, 57) and (currentJob == "police" or currentJob == "sheriff" or currentJob == "ambulance" or currentJob == "reporter") and not QBCore.Functions.GetPlayerData().metadata["isdead"]) then
...
function canPedBeUsed(ped)
    ...
    if IsPedTruckRobbery(ped) then
        return false
    end

if IsPedartrobberygallery(ped) then
    return false
end
