    ...
    for k, v in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
        if Config.UseTarget then
            ...
            zones[k]:onPlayerInOut(function(inside)
                if inside then
                    currentArea = k
                    exports['qb-drawtext']:DrawText(Lang:t("info.collect_coral_dt"))
                else
                    currentArea = 0
    ...
    for i = 1, #Config.SellLocations do
        ...
        if Config.UseTarget then
            ...
            zone:onPlayerInOut(function(inside)
                if inside then
                    inSellerZone = true
                    exports['qb-drawtext']:DrawText(Lang:t("info.sell_coral_dt"))
                else
                    inSellerZone = false
                    exports['qb-drawtext']:HideText()
                end
...
----- Sell Fish
RegisterNetEvent('qb-diving:sellfish', function()
    sellCoral()
end)
