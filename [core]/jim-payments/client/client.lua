...
RegisterNetEvent('jim-payments:client:PoliceCharge', function(data, outside)
	...
	newinputs[#newinputs+1] = { type = 'radio', name = 'billtype', text = Loc[Config.Lan].menu["type"], options = { { value = "bank", text = Loc[Config.Lan].menu["card"] } } }
	...
	if dialog then
		...
		TriggerServerEvent('jim-payments:server:ChargePolice', dialog.citizen, dialog.price, dialog.billtype, data.img, outside, gang)
	end
end)
