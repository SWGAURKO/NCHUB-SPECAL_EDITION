...
function getNPC()
 ...
 repeat
  ...
  if canPedBeUsed(ped) and distance < 3.0 and (distanceFrom == nil or distance < distanceFrom) and not exports['qb-interact']:HasPedMarked(ped) then
...
RegisterNetEvent('trunkgetin:event')
AddEventHandler('trunkgetin:event', function(getIn)
    ...
    if DoesEntityExist(vehicle) and GetVehicleDoorLockStatus(vehicle) == 1 and not kidnapped then
        exports.letters:start({
            ...
            if result then
                ...
                -- The player lost the letters game
            end
        end)
    else
        QBCore.Functions.Notify('You are not near a vehicle or the vehicle is locked or someone is in the vehicle', 'error')
    end
end)

RegisterNetEvent('trunkgetout:event')
AddEventHandler('trunkgetout:event', function(getIn)
  local pos = GetEntityCoords(GetPlayerPed(-1), false)
  local vehicle = GetClosestVehicle(pos.x, pos.y, pos.z, 5.0, 0, 71)
  if DoesEntityExist(vehicle) and GetVehicleDoorLockStatus(vehicle) == 1 and not kidnapped then
   if isInTrunk then
...
AddEventHandler('toggleTrunk', function(getIn)
 if getIn then
  exports["wsemotes"]:EmoteCommandStart("trunkgetin", 1)
 else
  exports["wsemotes"]:EmoteCommandStart("trunkgetout", 1)
 end
end)
...
Citizen.CreateThread(function()
 while true do
  ...
  local nearestVehicle = GetClosestVehicle(playerCoords.x, playerCoords.y, playerCoords.z, 3.0, 0, 71)

  if isCarrying then
    EnableAllControlActions(0)
    DisableControlAction(0, Keys['LEFTSHIFT'], true)
    DisableControlAction(0, Keys['F'], true)
    if IsControlJustPressed(1, 49) and nearestVehicle ~= 0 then
        QBCore.Functions.Notify('You cant get into a car while you are carrying someone', 'error')
    end
  end
 end
end)
