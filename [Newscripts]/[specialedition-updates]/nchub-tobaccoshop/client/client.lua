...
CreateThread(function()
        ...
        options = {
            {
                ...
                label = "Pick Up Tobacco",
                action = function(entity) 
                    if not entity then return false end 
                    TriggerEvent("nchub-tobaccoshop:client:PickUpTobacco", entity)
                end,
            },
    ...
    exports[Config.Target]:AddBoxZone('Shop', vector3(26.65, -1108.14, 29.48), 1.6, 1, {
        ...
        options = {
            {  
                event = "nchub-tobaccoshop:server:OpenShop",
                icon = "fas fa-shop",
                label = "Access Shop",
            },
        },
    ...
    exports[Config.Target]:AddBoxZone('craftmenu', vector3(28.47, -1110.23, 29.48), 0.5, 0.5, {
    name = 'craftmenu',
    heading=333,
    debugPoly=Config.Debug,
    minZ=25.48,
    maxZ=29.48
        ...
        options = {
            {  
                event = "nchub-tobaccoshop:client:OpenMenu",
...
RegisterNetEvent("nchub-tobaccoshop:client:PickUpTobacco", function(weedplant)
    ...
    QBCore.Functions.Progressbar("PickTobacco", "Picking Up Tobacco..", 5000, false, true, {
        ...
        TriggerServerEvent("nchub-tobaccoshop:server:PickUpTobacco")
        DeleteEntity(weedplant)
        ClearPedTasks(PlayerPedId())
    end) 
end)

RegisterNetEvent("nchub-tobaccoshop:server:OpenShop", function()
...
RegisterNetEvent("nchub-tobaccoshop:client:OpenMenu", function()
    local MenuProducts = {
        { 
            header = 'Make Cigar', 
    ...
    for k, v in pairs(Config.Products) do
        MenuProducts[#MenuProducts + 1] = {
            ...
            icon = k,
            params = {
                event = 'nchub-tobaccoshop:client:makingItems',
                args = {
                    item = k
                }
            }
...
RegisterNetEvent('nchub-tobaccoshop:client:makingItems', function(data) 
    QBCore.Functions.TriggerCallback("nchub-tobaccoshop:server:makingcigar", function(cb)
        if cb then 
            ...
            QBCore.Functions.Progressbar("makeitem", "Making " .. QBCore.Shared.Items[data.item].label, 4000, false, true, {
                ...
                TriggerServerEvent("nchub-tobaccoshop:server:GiveCigar", data.item)
