...
RegisterNetEvent('nchub-truckrob:Notify', function(message, type)
	if Config.UseCustomNotify then
        TriggerEvent('nchub-truckrob:CustomNotify',message, type)
...
CreateThread(function()
    ...
    if Config.UseThirdEye then
        local eye_options = {{
            event = "nchub-truckrob:RobberyCheck",
            icon = 'fas fa-phone',
                    ...
                    if Dist <= 50 then
                        ...
                        if Dist <= 3 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerEvent('nchub-truckrob:RobberyCheck')
                            end
                        end
                    end
                end
    ...
    if Config.UsePed then
        ...
        while true do
            ...
            if Dist <= 50 and not PedSpawned then
                TriggerEvent('nchub-truckrob:SpawnNPC',Config.StartPed,Config.StartModel)
                PedSpawned = true
...
RegisterNetEvent('nchub-truckrob:SpawnNPC',function(coords,model)
...
RegisterNetEvent('nchub-truckrob:RobberyCheck', function()
    if not Config.AllowLeoRob and isLawEnforcement then
        TriggerEvent('nchub-truckrob:Notify', Config.Lang['nocop'], Config.LangType['info'])
        return
    end
    if onjob then
        TriggerEvent('nchub-truckrob:Notify', Config.Lang['onjob'], Config.LangType['info'])
        return
    end
    local StartRobbery = true
    local hasItem = true
    TriggerEvent('nchub-truckrob:Notify', Config.Lang['checking'], Config.LangType['info'])
    if Config.RequireMinimumLEO then
        StartRobbery = false
        if Config.UseESX then
            ESX.TriggerServerCallback('nchub-truckrob:PoliceAvailable:ESX', function(cb)
                StartRobbery = cb
            end)                                    
        elseif Config.UseQBCore then
            QBCore.Functions.TriggerCallback('nchub-truckrob:PoliceAvailable:QBCore', function(cb)
                StartRobbery = cb
            end)
        end
        Wait(1000)
    ...
    if not StartRobbery then
        TriggerEvent('nchub-truckrob:Notify', Config.Lang['mincops'], Config.LangType['error'])
        return
    end
    if Config.RequireStartItem then
        hasItem = false
        if Config.UseESX then
            ESX.TriggerServerCallback('nchub-truckrob:itemTaken:ESX', function(cb)
                hasItem = cb
            end)                                    
        elseif Config.UseQBCore then
            QBCore.Functions.TriggerCallback('nchub-truckrob:itemTaken:QBCore', function(cb)
                hasItem = cb
            end)
        end
        Wait(1000)
    ...
    if hasItem then
        onjob = true
        TriggerServerEvent('nchub-truckrob:Server:Guards')
        TriggerEvent('nchub-truckrob:Notify', Config.Lang['startHeist'], Config.LangType['info'])
    else
        TriggerEvent('nchub-truckrob:Notify', Config.Lang['noitem'], Config.LangType['error'])
    end
end)

RegisterNetEvent('nchub-truckrob:Client:HesitSyncStarterStart', function(finish, start)
    BlipAdder(start, finish)
end)

RegisterNetEvent('nchub-truckrob:Client:HesitSyncCopsStart', function(finish, start)
    if isLawEnforcement then
        CreateThread(function()
            BlipAdder(start, finish)
        end)
        TriggerEvent('nchub-truckrob:Notify', Config.Lang['cop_notify'], Config.LangType['info'])
    end
end)

RegisterNetEvent('nchub-truckrob:Client:BeginRoute', function(finish, time, start, list)
    GaurdList[time] = {}
    ...
    TriggerEvent('nchub-truckrob:PoliceAlert', GetEntityCoords(vehicle))
    TriggerServerEvent('nchub-truckrob:Server:TruckSync', finish, time, start, id)
    onjob = false
...
RegisterNetEvent('nchub-truckrob:Client:TruckSync', function(time, inital, final, netid)
        ...
        if Config.UseThirdEye then
            if Config.ThirdEyeName == 'ox_target' then
                local options = {
                    {
                        onSelect = function()
                            TriggerServerEvent('nchub-truckrob:Server:ArmExplosion', obj, time)
                            LootAnim(obj)
                        end,
                        icon = 'fas fa-truck',
                    ...
                    {
                        onSelect = function()
                            TriggerEvent('nchub-truckrob:Loot', obj, netid, time)
                        end,
    ...
    while true do 
        ...
        if #(Pos-truckPos) <= 350 and DoesEntityExist(obj) then
            active = true
            TriggerEvent('nchub-truckrob:Notify', Config.Lang['truck_ping'], Config.LangType['info'])
            break
...
RegisterNetEvent('nchub-truckrob:Client:ArmExplosion', function(obj, number)
    EPlant[number] = true
...
RegisterNetEvent('nchub-truckrob:Loot', function(obj, netid, time)
    if not DoesEntityExist(obj) then return end
    TriggerServerEvent('nchub-truckrob:Server:ThirdEyeSync', netid, time)
    LootAnim(obj)
    TriggerServerEvent('nchub-truckrob:Server:HeistReward')
end)

RegisterNetEvent('nchub-truckrob:Client:ThirdEyeSync', function(netid, time)
    ELoot[time] = true
    local obj = NetworkGetEntityFromNetworkId(netid)
    while not DoesEntityExist(obj) do
        if DoesEntityExist(obj) then
...
function TruckSpawner(time, start, finsh, truck)
    ...
    if limit <= 0 then TriggerEvent('nchub-truckrob:Notify', src, Config.Lang['time_limit'], Config.LangType['error']) return false end
