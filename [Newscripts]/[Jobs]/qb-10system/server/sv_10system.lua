...
RegisterServerEvent("qb-10system:action")
AddEventHandler("qb-10system:action", function(data)
    if data.action == "error" then
        TriggerEvent('qb-10system:sendError', data.code)
    end
end)

RegisterServerEvent("qb-10system:add")
AddEventHandler("qb-10system:add", function()
    local data = {}
    for k,v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        local callSign = CallSigns[xPlayer.PlayerData.citizenid .. "|" .. jobName] or "NOT CALLSIGNS"
...
TriggerClientEvent("qb-10system:update", -1, data)
end)
...
AddEventHandler("TokoVoip:recordingstatus", function(sid, status)
 ...
 RegisterServerEvent("qb-10system:rank")
 AddEventHandler("qb-10system:rank", function(data)
