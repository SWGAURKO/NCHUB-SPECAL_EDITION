            ...
            if #(v.pedCoord - pos) < 3 then 
                ...
                if #(v.pedCoord - pos) < 2 then 
                    if IsControlJustReleased(0, 38) then 
                        SetNuiFocus(true,true)
                        if first then 
                            QBCore.Functions.TriggerCallback("qb-advancedFishing:getPlayerDetails", function(result)
                                first = false
...
RegisterNUICallback('takeRewards', function(data, cb)
	if takeRewardsSpamProtect < GetGameTimer() then 
		...
		QBCore.Functions.TriggerCallback("qb-advancedFishing:rewards", function(result)
			cb(result)
            if result then 
...
RegisterNUICallback('buyItem', function(data, cb)
    if buyItemSpamProtectx < GetGameTimer() then 
        ...
        QBCore.Functions.TriggerCallback("qb-advancedFishing:buyItem", function(result)
            cb(result)
        end, data)
    end
end)
...
RegisterNUICallback('sellItem', function(data, cb)
    if sellItemSpamProtectx < GetGameTimer() then 
        ...
        QBCore.Functions.TriggerCallback("qb-advancedFishing:sellItem", function(result)
            cb(result)
        end, data)
...
RegisterNUICallback('upgradeRod', function(data, cb)
	if upgradeSpamProtect < GetGameTimer() then 
		...
		QBCore.Functions.TriggerCallback("qb-advancedFishing:upgradeRod", function(result)
			cb(result)
		end, data)
	end
end)
...
addXP = function(xp)
	QBCore.Functions.TriggerCallback("qb-advancedFishing:addXP", function(result)
		if result then 
...
RegisterNetEvent('qb-advancedFishing:start')
AddEventHandler('qb-advancedFishing:start', function(rodLevel)
    if not fishing then
        ...
        if randomItem ~= nil then
            ...
            for k, v in pairs(Config.Tasks) do 
                if randomItem == v.itemName then 
                    TriggerServerEvent('qb-advancedFishing:addTaskCount', randomItem)
                    break
                end
            end
            TriggerServerEvent("qb-advancedFishing:addItem", randomItem)
            TriggerServerEvent('qb-battlepass:taskCountAdd:standart', 3, 1)
            TriggerServerEvent('qb-battlepass:taskCountAdd:premium', 4, 1)
        end
    end
end)
...
RegisterNetEvent('qb-advancedFishing:setBait')
AddEventHandler('qb-advancedFishing:setBait', function(baitType, baitName, label)
    if fishing then 
        if baitType == "fish" then -- Köepek Balığı
            bait = baitType
            SendNUIMessage({type = "setBait", bait = label})
            TriggerServerEvent('qb-advancedFishing:removeBait', baitName)
