...
RegisterNetEvent("row-beehives:setBeehives")
AddEventHandler("row-beehives:setBeehives", function()
    if not isPlacing then
        if inZone then
            ...
            if not hasShownMessage then
                ...
                placeObject("gate_beehive", function(pPlaced, pCoords, pHeading)
                if pPlaced then
                    ...
                    if finished == 100 then
                        ...
                        TriggerServerEvent("row-beekeeping:installHive", pCoords) 
...
AddEventHandler("row-beekeeping:checkBeehive", function()
    Wait(500)
...
AddEventHandler("row-beekeeping:harvestHive", function()
    Wait(500)
    ...
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        TriggerServerEvent("row-beekeeping:harvestHive", hiveId, hive.beequeen)
    end
end)
...
Citizen.CreateThread(function()
    for id,zone in ipairs(HiveZones) do
        exports[HiveConfig.PolyZone]:AddCircleZone("row-beekeeping:bee_zone", zone[1], zone[2])
    end
end)

AddEventHandler(HiveConfig.PolyZoneEnter, function(zone, data)
    if zone == "row-beekeeping:bee_zone" then
        inZone = true
...
local blips = {
    {title="Beekeeping Area", colour=82, id=285, x = 1530.7248, y = 1714.2497, z = 109.93022},}
    ...
    if bool then
          ...
          for k,v in pairs(blips) do
            zoneblip = AddBlipForRadius(v.x,v.y,v.z, 1200.0)
...
function showHiveMenu(pHiveId)
    ...
    if harvest >= 100 then
        beemenu[#beemenu+1] = {
            ...
            txt = "",
            params = {
                event = "row-beekeeping:harvestHive",
                args = pHiveId,
            },
        }
    end
