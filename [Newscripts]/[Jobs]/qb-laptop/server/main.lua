...
QBCore.Functions.CreateUseableItem(Config.LaptopDevice, function(source, item)
    ...
    if Player.Functions.GetItemByName(Config.LaptopDevice) then
        TriggerClientEvent('qb-laptop:client:openlaptop', source)
    end
end)
...
RegisterNetEvent('qb-laptop:server:LostAccess', function(app)
    local src = source
    if app == "boosting" then
        TriggerEvent("qb-laptop:server:QuitQueue", src)
    end
end)


RegisterNetEvent('qb-laptop:server:settings:set', function(setting)
    local src = source
...
RegisterNetEvent('qb-laptop:server:RemoveItem', function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
AddEventHandler('onResourceStart', function(resource)
    ...
    lib.versionCheck('Ejiy/qb-laptop')
end)
