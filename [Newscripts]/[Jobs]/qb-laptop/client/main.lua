                        ...
                        for k = 1, searches do
                            if app.bannedJobs[k] == playerJob then
                                if hadApp(app.app) then
                                    TriggerServerEvent('qb-laptop:server:LostAccess', app.app)
                                end
                                goto skip
...
RegisterNetEvent('qb-laptop:client:openlaptop', function()
    if haveItem(Config.LaptopDevice) then
        SetDisplay(true)
    end
end)

RegisterNetEvent('qb-laptop:client:CustomNotification', function(text, icon, color, iconbg, length)
    if haveItem(Config.LaptopDevice) then
        SendNUIMessage({
            action = "custom-notif",
            data = {
...
RegisterNUICallback('laptop/checkout', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('qb-laptop:server:checkout', function(result)
        if result == "bank" then
            cb({
                status = 'error',
...
RegisterNUICallback('setting/save', function(data, cb)
    ...
    cb("ok")
    TriggerServerEvent("qb-laptop:server:settings:set", data["setting"])
end)
