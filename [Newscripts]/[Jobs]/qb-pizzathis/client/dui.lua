...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	local p = promise.new()
	QBCore.Functions.TriggerCallback('qb-pizzathis:Server:duiList', function(cb) p:resolve(cb) end)
	duiList = Citizen.Await(p)
	if inZone then
		for k, v in pairs(duiList) do TriggerEvent("qb-pizzathis:ChangeDUI", { url = v.url, tex = k, texd = v.texd, size = v.size }) end
	end
end)
...
CreateThread(function()
	...
	PizzathisDUI:onPlayerInOut(function(isPointInside)
		if isPointInside then inZone = true
			...
			local p = promise.new() QBCore.Functions.TriggerCallback('qb-pizzathis:Server:duiList', function(cb) p:resolve(cb) end)
			duiList = Citizen.Await(p)
			for k, v in pairs(duiList) do TriggerEvent("qb-pizzathis:ChangeDUI", { url = v.url, tex = k, texd = v.texd, size = v.size }) end
		else inZone = false
			...
			for k, v in pairs(duiList) do
				v.tex = k
				TriggerEvent("qb-pizzathis:ClearDUI", v)
			end
			dui = {}
			duiList = {}
		end
	...
	for k, v in pairs(PresetList) do
		...
		exports['qb-target']:AddCircleZone(v["tex"].texn, v["coords"], v["radius"], { name=v["tex"].texn, debugPoly=Config.Debug, useZ=true, },
			{ options = {
					{ type = "client", event = "qb-pizzathis:DuiSelect", icon = "fas fa-circle-check", label = Loc[Config.Lan].targetinfo["add_image"], job = JobGrade, tex = v["tex"].texn, size = v["tex"].size, texd = v["tex"].texd },
					{ type = "server", event = "qb-pizzathis:Server:ChangeDUI", icon = "fas fa-circle-minus", label = Loc[Config.Lan].targetinfo["reset_image"], job = JobGrade, tex = v["tex"].texn, texd = v["tex"].texd },
					{ type = "server", event = "qb-pizzathis:Server:ClearDUI", icon = "fas fa-circle-xmark", label = Loc[Config.Lan].targetinfo["clear_image"], job = JobGrade, tex = v["tex"].texn, texd = v["tex"].texd, url = "-" },
			}, distance = distance })
	end
end)

RegisterNetEvent("qb-pizzathis:DuiSelect", function(data)
    ...
    if dialog then
        ...
        if searchFound then TriggerServerEvent('qb-pizzathis:Server:ChangeDUI', data) end
    end
end)

RegisterNetEvent("qb-pizzathis:ChangeDUI", function(data)
	...
	if (dui[tostring(data.tex)] or inZone) and tostring(data.url) ~= "-" then
		TriggerEvent("qb-pizzathis:ClearDUI", data)
		--Add DUI to table
...
RegisterNetEvent("qb-pizzathis:ClearDUI", function(data)
