...
RegisterServerEvent('qb-pizzathis:OpenBox', function(source, item)
	local src = source
	...
	TriggerEvent("qb-pizzathis:server:toggleItem", false, item, 1, src)
	Wait(600)
	--This gives the slices
	TriggerEvent("qb-pizzathis:server:toggleItem", true, QBCore.Shared.Items[slices].name, 6, src)

end)

---Crafting
RegisterServerEvent('qb-pizzathis:Crafting:GetItem', function(ItemMake, craftable)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	...
	if craftable then
		...
		for k, v in pairs(craftable[ItemMake]) do TriggerEvent("qb-pizzathis:server:toggleItem", false, tostring(k), v, src) end
	end
	--This should give the item, while the rest removes the requirements
	TriggerEvent("qb-pizzathis:server:toggleItem", true, ItemMake, amount, src)
end)

RegisterNetEvent('qb-pizzathis:server:toggleItem', function(give, item, amount, newsrc)
	...
	if not give then
		...
		else TriggerEvent("qb-pizzathis:server:DupeWarn", item, src) end -- if not boot the player
	elseif give then
...
RegisterNetEvent("qb-pizzathis:server:DupeWarn", function(item, newsrc)
	local src = newsrc or source
