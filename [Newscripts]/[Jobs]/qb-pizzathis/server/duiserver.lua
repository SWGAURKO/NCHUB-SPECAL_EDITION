...
RegisterServerEvent("qb-pizzathis:Server:ChangeDUI", function(data)
	...
	TriggerClientEvent("qb-pizzathis:ChangeDUI", -1, data)
end)
RegisterServerEvent("qb-pizzathis:Server:ClearDUI", function(data)
	if data.url == "-" then duiList[data.tex].url = "-" end
	-- Clear the DUI from loading
	TriggerClientEvent("qb-pizzathis:ClearDUI", -1, data)
	--duiList[tostring(data.tex)].url = ""
end)

QBCore.Functions.CreateCallback('qb-pizzathis:Server:duiList', function(source, cb) cb(duiList) end)