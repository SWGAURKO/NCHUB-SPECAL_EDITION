...
local function SpawnCowsAndTasks(id)
    ...
    TriggerServerEvent("qb-farming-insert-cow", NetworkGetNetworkIdFromEntity(veh))
    TriggerServerEvent("qb-farming-insert-cow", NetworkGetNetworkIdFromEntity(cowped))
    CreateThread(function()
        while JobStarted do
            local ms = 250
                            ...
                            if IsControlJustPressed(0, 38) and not isBussy then
                                QBCore.Functions.TriggerCallback("qb-farming-check-item", function(check)
                                    if check then
                                        ...
                                        ClearPedTasks(ped)
                                        Wait(750)
                                        TriggerEvent('qb-farming-carry-object', MainShared.ChurnProp["model"], MainShared.ChurnProp["bone"], MainShared.ChurnProp["x"],MainShared.ChurnProp["y"], MainShared.ChurnProp["z"], MainShared.ChurnProp["xR"], MainShared.ChurnProp["yR"] ,MainShared.ChurnProp["zR"], MainShared.ChurnProp["animDict"], MainShared.ChurnProp["animName"])
                                        isBussy = false
                                    else
                        ...
                        if not isBussy then
                            ...
                            if dist <= 1.5 then
                                ...
                                if IsControlJustPressed(0, 38) and not isBussy then
                                    milkelimde = false
                                    isBussy = true
                                    TriggerServerEvent('qb-farming-delete-churn')
                                    cancelCarryAnim()
                                    Wait(50)
            ...
            if Current_Stage == 5 then
                ...
                if dist < 3.0 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ClearCustomBlips()
                        TriggerServerEvent('qb-farming-cowjob-finished', id)
                        DeleteVehicle(veh)
                        DeleteEntity(cowped)
                        ExecuteCommand('yenile')
                        JobStarted = false
                        Current_Stage = 0 
                        TriggerServerEvent('qb-farming-receive-milk')
...
local function StartCow(id)
    ...
    QBCore.Functions.TriggerCallback('qb-farming-start-cowfield', function(canStart)
        if canStart then
            ...
            TriggerServerEvent('qb-farming-cow-started', id)
        else
            MainShared.Notify(Lan.SomethingWentWrong, 'error')
        end
    end, id)
end RegisterNetEvent('qb-farming-start-cow', StartCow)