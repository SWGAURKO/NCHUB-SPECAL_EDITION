...
local function StartMelonTask(id)
    ...
    TriggerServerEvent("qb-farming-insert-melon", NetworkGetNetworkIdFromEntity(youga))
                                ...
                                if #(GetEntityCoords(PlayerPedId()) - vector3(v.c.x ,v.c.y,v.c.z)) <= 0.7 then
                                    if MelonShared.KeyListener then
                                        ...
                                        if IsControlJustPressed(0, 38) then
                                            QBCore.Functions.TriggerCallback("qb-farming-check-item", function(check)
                                                if check then
                                                    bekle = true
                                                    v.b = true
                                                    RakerAnim()
                            ...
                            if #(GetEntityCoords(PlayerPedId()) - vector3(v.c.x ,v.c.y,v.c.z)) <= 0.75 then
                                if MelonShared.KeyListener then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        QBCore.Functions.TriggerCallback("qb-farming-check-item", function(check)
                                            if check then
                                                QBCore.Functions.TriggerCallback("qb-farming-check-item2", function(check2)
                                                    if check2 then
                                                        bekle = true
                                                        v.b = true
                                                        PlantAnim()
                ...
                if Current_Stage == 3 then
                    ...
                    if Max_Veri == Yapilan_Veri then
                        ...
                        for k,v in pairs(MelonShared.Fields[id]) do
                            ...
                            Max_Veri += 1
                            TriggerServerEvent("qb-farming-insert-melon", NetworkGetNetworkIdFromEntity(melon))
                        end
                        Wait(250)
                        bekle = false
                    end
                                        ...
                                        if IsControlJustPressed(0, 38) then
                                            QBCore.Functions.TriggerCallback("qb-farming-check-item", function(check)
                                                if check then
                                                    ...
                                                    karpuzelimde = true
                                                    Wait(500)
                                                    TriggerEvent('qb-farming-carry-object', MainShared.MelonProp["model"], MainShared.MelonProp["bone"], MainShared.MelonProp["x"],MainShared.MelonProp["y"], MainShared.MelonProp["z"], MainShared.MelonProp["xR"], MainShared.MelonProp["yR"] ,MainShared.MelonProp["zR"], MainShared.MelonProp["animDict"], MainShared.MelonProp["animName"])
                        ...
                        if dist < 0.75 then
                            if MelonShared.KeyListener then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    Wait(1100)
                                    FreezeEntityPosition(PlayerPedId(), false)
                                    TriggerEvent('qb-farming-stop-carry')
                                    ClearPedTasks(PlayerPedId())
                                    karpuzelimde = false
                                    Yapilan_Veri += 1
                                    MainShared.Notify(string.format(Lan.Maded, Yapilan_Veri,Max_Veri), 'primary')
                        ...
                        if MelonShared.KeyListener then
                            ...
                            if IsControlJustPressed(0, 38) then
                                if IsPedInAnyVehicle(PlayerPedId()) then
                                    ...
                                    TriggerServerEvent('qb-farming-melonjob-finished', id, money)
                                    TriggerServerEvent('qb-farming-receive-melon')
                            ...
                            if IsPedInAnyVehicle(PlayerPedId()) then
                                TriggerServerEvent('qb-farming-melonjob-finished', id)
                                TriggerServerEvent('qb-farming-receive-melon')
                                MainShared.Notify(string.format(Lan.ReceiveMelon, MelonShared.CuttedMelon), 'primary')
                                DeleteVehicle(youga)
                                MainShared.ResetClothe()
...
local function StartMelon(id)
    id = tonumber(id)
    if id == nil then print('[qb-farming:ERR]: Field Id Is Nil') return end
    QBCore.Functions.TriggerCallback('qb-farming-start-melonfield', function(canStart)
        if canStart then
            JobStarted = true
            MainShared.Notify(Lan.JobStarted, 'primary')
            StartMelonTask(id)
            TriggerServerEvent('qb-farming-melon-started', id)
        else
            MainShared.Notify(Lan.SomethingWentWrong, 'error')
        end
    end, id)
end RegisterNetEvent('qb-farming-start-melon', StartMelon)