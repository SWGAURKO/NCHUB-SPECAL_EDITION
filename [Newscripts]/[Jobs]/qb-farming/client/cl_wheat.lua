...
local function StartWheatTask(id)
    ...
    TriggerServerEvent("qb-farming-insert", NetworkGetNetworkIdFromEntity(tractor))
    TriggerServerEvent("qb-farming-insert", NetworkGetNetworkIdFromEntity(harvester))
    TriggerServerEvent("qb-farming-insert", NetworkGetNetworkIdFromEntity(raker))
    TriggerServerEvent("qb-farming-insert", NetworkGetNetworkIdFromEntity(trailer))
            ...
            if not bekle then
                ...
                if Current_Stage == 2 then
                    ...
                    if Max_Veri == Yapilan_Veri then
                        ...
                        for k,v in pairs(WheatShared.WheatFields[id]) do
                            ...
                            Max_Veri += 1
                            TriggerServerEvent("qb-farming-insert", NetworkGetNetworkIdFromEntity(wheat))
                        end
                        Wait(250)
                        bekle = false
                    end
                ...
                if Current_Stage == 3 then
                    ...
                    if Balya_Count == 8 and OLUSTURULAN_BALYA ~= 8 then
                        ...
                        OLUSTURULAN_BALYA +=1
                        TriggerServerEvent("qb-farming-insert", NetworkGetNetworkIdFromEntity(balya))
                    end
                ...
                if Current_Stage == 6 then
                    ...
                    if #(GetEntityCoords(tractor) - vector3(WheatShared.DropTractorLoc.x, WheatShared.DropTractorLoc.y, WheatShared.DropTractorLoc.z)) <= 2.0 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            Current_Stage = 0 
                            ClearWheatBlips()
                            TriggerServerEvent('qb-farming-wheat-finished', id)
                            DeleteVehicle(tractor)
                            DeleteVehicle(harvester)
                            DeleteVehicle(raker)
                            DeleteVehicle(trailer)
                            TriggerServerEvent('qb-farming-receive-wheat')
...
local function StartWheat(id)
    ...
    QBCore.Functions.TriggerCallback('qb-farming-start-wheatfield', function(canStart)
        if canStart then
            JobStarted = true
            MainShared.Notify(Lan.JobStarted, 'primary')
            StartWheatTask(id)
            TriggerServerEvent('qb-farming-wheat-started', id)
        else
            MainShared.Notify(Lan.SomethingWentWrong, 'error')
        end
    end, id)
end RegisterNetEvent('qb-farming-start-wheat', StartWheat)
