...
local function OpenUI(x)
    if x == "Wheat" then
        QBCore.Functions.TriggerCallback("qb-farming-get-wheat-fields", function(data)
            SendNUIMessage({
                action = 'open',
                config = Config[x],
                type = x,
    ...
    elseif x == "Cow" then
        QBCore.Functions.TriggerCallback("qb-farming-get-cow-fields", function(data)
            SendNUIMessage({
                action = 'open',
                config = Config[x],
                type = x,
    ...
    elseif x == "Melon" then
        QBCore.Functions.TriggerCallback("qb-farming-get-melon-fields", function(data)
            SendNUIMessage({
                action = 'open',
                config = Config[x],
                type = x,
    ...
    elseif x == "Pumpkin" then
        QBCore.Functions.TriggerCallback("qb-farming-get-pumpkin-fields", function(data)
            SendNUIMessage({
                action = 'open',
                config = Config[x],
                type = x,
...
local function BuyFunction(data, cb)
    ...
    QBCore.Functions.TriggerCallback('qb-farming-buy', function(bought)
        if bought then
            cb(true)
        else
            cb(false)
...
local function SellItem(data)
    TriggerServerEvent('qb-farming-sell', data)
...
end RegisterNetEvent('qb-farming-stop-carry', removeAttachedcarryObject)

RegisterNetEvent('qb-farming-carry-object', function(attachModelSent, boneNumberSent, x, y, z, xR, yR, zR, Anim1, Anim2)
    anim1 = Anim1
    anim2 = Anim2
	removeAttachedcarryObject()
...
local function StartJob(data, cb)
    ...
    if current_job == 'Wheat' then
        ...
        TriggerEvent('qb-farming-start-wheat', data.id)
        cb(true)
        SetNuiFocus(false, false)
    elseif current_job == 'Cow' then
        if data.id == nil then return end
        TriggerEvent('qb-farming-start-cow', data.id)
        cb(true)
        SetNuiFocus(false, false)
    elseif current_job == 'Melon' then
        if data.id == nil then return end
        TriggerEvent('qb-farming-start-melon', data.id)
        cb(true)
        SetNuiFocus(false, false)
    elseif current_job == 'Pumpkin' then
        if data.id == nil then return end
        TriggerEvent('qb-farming-start-pumpkin', data.id)
        cb(true)
        SetNuiFocus(false, false)
    else
        cb(false)
