...
local function StartPumpkinTask(id)
    ...
    TriggerServerEvent("qb-farming-insert-pumpkin", NetworkGetNetworkIdFromEntity(youga))
                ...
                if Current_Stage == 3 then
                    ...
                    if Max_Veri == Yapilan_Veri then
                        ...
                        for k,v in pairs(PumpkinShared.Fields[id]) do
                            ...
                            Max_Veri += 1
                            TriggerServerEvent("qb-farming-insert-pumkin", NetworkGetNetworkIdFromEntity(pumkin))
                        end
                        Wait(250)
                        bekle = false
                    end
                        ...
                        if PumpkinShared.KeyListener then
                            ...
                            if IsControlJustPressed(0, 38) then
                                if IsPedInAnyVehicle(PlayerPedId()) then
                                    TriggerServerEvent('qb-farming-pumpkinjob-finished', id)
                                    TriggerServerEvent('qb-farming-receive-pumpkin')
                            ...
                            if IsPedInAnyVehicle(PlayerPedId()) then
                                ...
                                TriggerServerEvent('qb-farming-pumpkinjob-finished', id, money)
                                TriggerServerEvent('qb-farming-receive-pumpkin')
                                MainShared.Notify(string.format(Lan.ReceivePumpkin, PumpkinShared.CuttedPumpkin), 'primary')
                                DeleteVehicle(youga)
                                MainShared.ResetClothe()
...
local function StartPumpkin(id)
    ...
    QBCore.Functions.TriggerCallback('qb-farming-start-pumpkinfield', function(canStart)
        if canStart then
            JobStarted = true
            MainShared.Notify(Lan.JobStarted, 'primary')
            StartPumpkinTask(id)
            TriggerServerEvent('qb-farming-pumpkin-started', id)
        else
            MainShared.Notify(Lan.SomethingWentWrong, 'error')
        end
    end, id)
end RegisterNetEvent('qb-farming-start-pumpkin', StartPumpkin)