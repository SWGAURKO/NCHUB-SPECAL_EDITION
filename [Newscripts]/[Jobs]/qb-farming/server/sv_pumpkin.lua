...
QBCore.Functions.CreateCallback('qb-farming-get-pumpkin-fields', function(source, cb)
    cb(Pumpkin_Fields)
end)

QBCore.Functions.CreateCallback('qb-farming-start-pumpkinfield', function(source, cb, FieldID)
    local src = source
...
RegisterServerEvent('qb-farming-pumpkin-started', function(FieldID)
    Pumpkin_Fields[FieldID] = true
    fields[source] = FieldID
    DeleteTable[source] = {}
end)

RegisterServerEvent('qb-farming-insert-pumpkin', function(value)
    local src = source
    for i = 1, 10, 1 do 
        Wait(0)
...
RegisterServerEvent('qb-farming-pumpkinjob-finished', function(FieldID)
    local src = source
    Entitydeleter(src)
    Pumpkin_Fields[FieldID] = false
end)

RegisterNetEvent('qb-farming-receive-pumpkin', function()
    local src = source
