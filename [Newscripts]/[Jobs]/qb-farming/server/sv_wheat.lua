...
QBCore.Functions.CreateCallback('qb-farming-get-wheat-fields', function(source, cb)
    cb(Wheat_Fields)
end)

QBCore.Functions.CreateCallback('qb-farming-start-wheatfield', function(source, cb, FieldID)
    local src = source
...
RegisterServerEvent('qb-farming-wheat-started', function(FieldID)
    Wheat_Fields[FieldID] = true
    fields[source] = FieldID
    DeleteTable[source] = {}
end)

RegisterServerEvent('qb-farming-insert', function(value)
    local src = source
    for i = 1, 10, 1 do 
        Wait(0)
...
RegisterServerEvent('qb-farming-wheat-finished', function(FieldID)
    local user = QBCore.Functions.GetPlayer(source)
    Wheat_Fields[FieldID] = false
end)

RegisterNetEvent('qb-farming-receive-wheat', function()
    local src = source
