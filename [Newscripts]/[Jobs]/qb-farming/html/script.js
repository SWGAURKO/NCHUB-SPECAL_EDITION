    ...
    window.addEventListener("message", function(event) {
        ...
        switch (event.data.action) {
            case "open":
                ...
                break;
            case "close":
                $.post("https://qb-farming/ResetClothes");
                $(".shopingbarlist").html("");
                SelectedItems = [];
                Amounts = [];
                AllItems = [];
...
$(document).on('click', '.sellbuton', function(e) {
    ...
    if (currshop == 'buy') {
        ...
        $.post("https://qb-farming/SellItem", JSON.stringify({
            price: price,
            item: itemCode
        }));
    }
...
$(document).on('click', '.farmingsell', function(e) {
    e.preventDefault()
    if (currshop != 'sell') {
        $.post("https://qb-farming/GetSellConfig", function(data) {
...
$(document).on('click', '.farmingbuy', function(e) {
    ...
    if (currshop != 'buy') {
        currshop = "buy";
        $.post("https://qb-farming/GetShopConfig", function(data) {
...
$(document).on('click', ".kutucukbar", function(e) {
    e.preventDefault()
    let id = $(this).attr("id")
    $.post("https://qb-farming/StartJob", JSON.stringify({
        id: id
    }), function(cb) {
        if (cb) {
            FadeOut(".budaykontaynir", 350)
...
$(document).on('click', ".clotinghbir", function(e) {
    e.preventDefault()
    $.post("https://qb-farming/WearClothes");
});

$(document).on('click', '.cashdiv', function(e) {
    e.preventDefault()
    $.post("https://qb-farming/buy", JSON.stringify({
