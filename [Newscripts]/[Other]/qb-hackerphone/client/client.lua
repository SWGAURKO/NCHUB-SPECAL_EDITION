...
RegisterNetEvent("qb-hackerphone:client:openphone",function(name)
...
RegisterNetEvent("qb-hackerphone:client:targetinfornui", function(targetinfo)
    SendNUIMessage({nuimessage = "userlists", uinfo = targetinfo})
end)

RegisterNetEvent("qb-hackerphone:client:centralchip", function()
	...
	if dist < 2 then 
		...
		exports['ps-ui']:Scrambler(function(success)
			if success then
				...
				TriggerServerEvent('qb-hackerphone:server:removeitem',"centralchip")
			else
				ClearPedTasks(ped)
...
RegisterNetEvent("qb-hackerphone:client:vehicletracker", function()
		...
		if #(pos - vehpos) < 2 then
			...
			exports['ps-ui']:Circle(function(success)
				if success then
					...
					TriggerServerEvent('qb-hackerphone:server:removeitem',"tracker")
				else
					ClearPedTasks(ped)
					QBCore.Functions.Notify('Failed to connect', "error")
				end
...
RegisterNetEvent("qb-hackerphone:client:notify", function()
    SendNUIMessage({nuimessage = 'error'})
end)

RegisterNUICallback("qb-hackerphone:nuicallback:targetinformation", function()
    TriggerServerEvent('qb-hackerphone:server:targetinformation')
end)

RegisterNUICallback("qb-hackerphone:broken:vehicle", function(vehicle)
	local vehpos = GetEntityCoords(vehicle)
	AddExplosion(vehpos.x, vehpos.y, vehpos.z, 7, 0.5, true, false, true)
end)

-- RegisterNUICallback('qb-hackerphone:nuicallback:blackout', function()
--     TriggerServerEvent('qb-weathersync:server:toggleBlackout')
-- end)

RegisterNUICallback("qb-hackerphone:nuicallback:cam", function(camid)
	TriggerEvent("police:client:ActiveCamera", tonumber(camid))
end)

RegisterNUICallback("qb-hackerphone:nuicallback:escape", function()
