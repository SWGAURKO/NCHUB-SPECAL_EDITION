...
$("#hide").click(function() {
    if (clientId) {
        $.post('https://qb_tattooshop/hide', JSON.stringify({clientId: clientId}));
        clientId = undefined
    } else {
        $.post('https://qb_tattooshop/hide', JSON.stringify({}));
    }
})
$(document).on('click', '#remove', function(e){
    ...
    if (clientId) {
        $.post('https://qb_tattooshop/remove', JSON.stringify({
            clientId: clientId,
...
$(document).on('click', '#buy', function(e){
    ...
    var id = $(this).data("tattooid")
    if (clientId) {
        $.post('https://qb_tattooshop/buy', JSON.stringify({
            clientId: clientId,
            collection: collection,
            id: id
        }));
...
$(document).on('click', '#show', function(e){
    ...
    if (clientId) {
        $.post('https://qb_tattooshop/change', JSON.stringify({
            item: id,
            collection: collection,
            clientId: clientId,
        }));
...
function loadCategory(typeDlc, numberOfSelected) {
    ...
    if (clientId) {
        $.post('https://qb_tattooshop/switchCategory', JSON.stringify({clientId: clientId}));
    } else {
        $.post('https://qb_tattooshop/switchCategory', JSON.stringify({}))
    }
    $("#panelInside").empty()
    $("#panelInside").html(`
        <div>
