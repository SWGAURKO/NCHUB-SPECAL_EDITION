...
RegisterNetEvent('qb-dumpster:server:ResetEntity', function(entity)
    entities[entity] = 0
end)
...
RegisterNetEvent('qb-dumpster:server:SetEntity', function(netId, isFinished)
    entities[netId] = 0
...
QBCore.Functions.CreateCallback('qb-dumpster:server:getEntityState', function(source, cb, netId)
...
AddEventHandler('onResourceStop', function(resName)
    ...
    for _,v in pairs(entities) do
        if v == -1 then
            TriggerClientEvent('qb-dumpster:client:ResetEntity', -1, _)
        end
    end
end)