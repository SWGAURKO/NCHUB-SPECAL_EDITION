...
local Locations = {
    ...
    vector3(348.96121, -800.1359, 4.7067918), -- APARTMENTS
    ...
    vector3(-264.015, -957.306, 2.2281019),
    vector3(153.90013, -3011.345, 7.040883),
    vector3(9.6564426, 528.7453, 170.61875),
    vector3(-263.6522, -957.2541, -57.77187),
    vector3(386.42065, 799.71099, 187.46153),
    vector3(810.59552, -759.8251, 31.26589),
    vector3(362.16964, -1593.715, 25.448917),
    vector3(-587.569, -1050.615, 22.344198),
    vector3(-1192.009, -897.6768, 13.995315),
    vector3(154.0346, -3012.649, 7.0408902),
    vector3(841.10626, -823.5354, 26.332571),
    vector3(362.16964, -1593.715, 25.448917),
    vector3(853.62048, -1312.718, 28.244939),
    vector3(1539.3164, 810.02716, 77.667747),
}


RegisterNetEvent("qb-outfits:client:SetClothing",  function(data)
    pcall(function()
...
RegisterNetEvent("qb-outfits:client:saveOutfit", function(slot)
    ...
    TriggerServerEvent("qb-outfits:server:createOutfit", slot, temp_data)
end)
...
RegisterNetEvent("qb-outfits:resetModel", function(model)
    local playerCoords = GetEntityCoords(PlayerPedId())
    local found = false
    ...
    TriggerEvent('qb-admin:client:SetModel', tostring(model))
end)

RegisterNetEvent("qb-outfits:client:openMenu", function(data)
    local playerCoords = GetEntityCoords(PlayerPedId())
    local found = false
    local InHouse = exports['qs-housing']:insideHouse()

    for _, coords in pairs(Locations) do
        if(#(coords - playerCoords) < 4.5) or InHouse then
            found = true
            break
        end
    end
            ...
            sub_menu = {
                {
                    ...
                    event = "qb-outfits:server:setOutfit",
                    eventType = "server",
                    args = {
                        ["slot"] = index
                    },
                ...
                },
                {
                    ...
                    event = "qb-outfits:server:delete",
                    eventType = "server",
                    args = {
                        ["slot"] = index
                    },
    ...
    exports['qb-outfitsmenu']:openMenu(menu)


end)


RegisterNetEvent("qb-outfits:client:OpenTrunk", function(data)
