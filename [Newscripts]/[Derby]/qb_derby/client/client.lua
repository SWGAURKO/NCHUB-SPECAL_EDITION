        ...
        if ArenaBusy then
            ...
            if veh > 0 then
                ...
                if hit > 0 and entityHit > 0 then
                    ...
                    TriggerServerEvent("qb_derby:SetPlayerHit", _GetPlayerServerID(driverPed))
                end
            end
        else
            Wait(1000)
    ...
    while true do
        ...
        if ArenaAPI:IsPlayerInArena(GetPlayerActiveArena()) and ArenaBusy and InCar then
            if #(GetEntityCoords(PlayerPedId()) - Config.ArenaLocation) > Config.ArenaDistance or GetEntityCoords(PlayerPedId()).z < Config.MinimumZLevel then
                TriggerServerEvent("qb_derby:CountLife")
...
RegisterNetEvent("qb_derby:ArenaEnded")
AddEventHandler("qb_derby:ArenaEnded", function(sorted)
    ...
    TriggerServerEvent("qb_derby:RemovePlayerFromWorld")
    InCar = false
end)

-- will display player score
-- and count killstreak
RegisterNetEvent("qb_derby:DisplayScore")
AddEventHandler("qb_derby:DisplayScore", function()
    if ArenaBusy then
        SendNUIMessage({ type = "addScore", })
...
RegisterNetEvent("qb_derby:ArenaStarted")
AddEventHandler("qb_derby:ArenaStarted", function(data)
    SetDisplay(true)
