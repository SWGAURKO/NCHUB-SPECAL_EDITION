...
for k, v in pairs(Config.DerbyArenaNameList) do
    ...
    arena.OnArenaStart(function(data)
        ...
        for k, v in pairs(arena.GetPlayerList()) do
            ...
            TriggerClientEvent("qb_derby:ArenaStarted", k, SpawnData)
        end
    end)
    ...
    arena.OnArenaEnd(function(data)
        ...
        for k, v in pairs(arena.GetPlayerList()) do
            TriggerClientEvent("qb_derby:ArenaEnded", k, SortedList)
        end
...
RegisterNetEvent("qb_derby:RemovePlayerFromWorld")
AddEventHandler("qb_derby:RemovePlayerFromWorld", function()
    SetPlayerRoutingBucket(source, 0)
end)


RegisterNetEvent("qb_derby:CountLife")
AddEventHandler("qb_derby:CountLife", function()
        ...
        if arena.IsPlayerInArena(_source) then
            if arena.PlayerScoreExists(_source, "HitBy") then
                ...
                if target ~= _source then
                    ...
                    TriggerClientEvent("qb_derby:DisplayScore", target, _source)
...
RegisterNetEvent("qb_derby:SetPlayerHit")
AddEventHandler("qb_derby:SetPlayerHit", function(playerID)
    local _source = source
    for k, arena in pairs(CachedArenas) do
